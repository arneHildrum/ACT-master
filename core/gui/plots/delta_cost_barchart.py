...
class DeltaCostBarchart(BaseACTPlot):
    """
    A horizontal stacked bar chart showing cost differences by model type.

    This plot displays cost differences between baseline
    ...
    def __init__(self, base_sim, delta_sims):
        """
        Initialize the Delta Cost Bar Chart plot.
    ...
    def make_stacked_barchart_data(self, sim):
        """
        Creates dataset for a stacked barchart from cost results.
        ...
        total_cost = 0 * dollar
        for cost_result in sim.cost_analyzer.cost_by_device.values():
            total_cost += cost_result.total

        for device_name, cost_result in sim.cost_analyzer.cost_by_device.items():
            ...
            if hasattr(sim, "bom") and sim.bom and device_name in sim.bom.devices:
                ...
                # Convert cost to cost unit
                cost_value = cost_result.total.to(self.cost_unit)
                cost_magnitude = cost_value.m

                # Skip devices with near-zero cost
                if abs(cost_magnitude) < 1e-5:
                    continue

                # Format cost value and percentage
                fcost_val = f"{cost_magnitude:.2f} {self.fcost_unit}"
                if total_cost.m > 0:
                    fpercent = f"{(cost_magnitude / total_cost.to(self.cost_unit).m * 100):.2f}"
                else:
                    fpercent = "0.00"
        ...
        for category, barchart_items in barchart_data.items():
            ...
            if not barchart_items:
                ...
                for barchart_item in barchart_items:
                    trace_label = f"{barchart_item['device_name']}<br>{barchart_item['fcost_val']}<br>{barchart_item['fpercent']}%"

                    barchart_traces[category].append(
                        go.Bar(
                            y=[category_label],
                            x=[barchart_item["cost_value"].m],
            ...
            # Add the total cost annotation for each stacked bar
            category_cost_total = (
                f"{barchart_traces_current_val[category]:.2f} {self.fcost_unit}"
            )
            barchart_annotations.append(
                go.layout.Annotation(
                    x=barchart_traces_current_val[category],
                    y=category_label,
                    text=category_cost_total,
                    showarrow=False,
                    xanchor="left",
                    yanchor="middle",
                )
        ...
        for category in all_categories:
            ...
            for i, item_1 in enumerate(barchart_items_1):
                if i < len(barchart_items_2):
                    ...
                    if abs(item_1["cost_value"].m - item_2["cost_value"].m) < 1e-5:
    ...
    def get_html(self):
        """
        Generate HTML content for the horizontal stacked bar chart showing cost differences.
        ...
        bar_plot_layout = go.Layout(
            barmode="stack",
            xaxis={"title": f"Cost ({self.fcost_unit})"},
            yaxis={"title": "Cost Category (Model Type)"},
            showlegend=False,
            bargap=0.1,
